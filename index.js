  ...
  if (kufur) {
    ...
    if (kufurler.some((alo) => message.content.toLowerCase().includes(alo))) {
      ...
      message.channel.send(
        `Hey <@${message.author.id}>, Bu Sunucuda Küfür Engel Sistemi Aktif! `
      );
    }
  }
});
client.on("messageCreate", (message) => {
  ...
  if (reklamlar) {
    ...
    if (linkler.some((alo) => message.content.toLowerCase().includes(alo))) {
      message.delete();
      message.channel.send(
        `Hey <@${message.author.id}>, Bu Sunucuda Reklam Engel Sistemi Aktif! `
      );
    }
  }
});
...
client.on("guildMemberAdd", async (member) => {
  ...
  client.channels.cache
    .get(sayac.kanal)
    .send(
      "<a:duyur:1084953665049219112> Hoşgeldin **" +
        ...
        "` Kişi Olmamıza Son `" +
        kalan +
        "` Kişi Kaldı! <a:psoprime_emoji:1084585759195021422>"
    );
});
client.on("guildMemberRemove", async (member) => {
  ...
  client.channels.cache
    .get(sayac.kanal)
    .send(
      "<:cikis:1084586075017719898> Görüşürüz **" +
        member.user.username +
        "** Senin Yüzünden `" +
        member.guild.memberCount +
        "` Kişi Olduk! <a:psoprime_emoji:1084585759195021422>"
    );
});
...
client.on("guildMemberAdd", async (member) => {
  ...
  client.channels.cache
    .get(otorol.kanal)
    .send(
      "<:giris:1084586043870806036> **" +
        member.user.tag +
        "** Kullanıcı Katıldı! Gerekli Rolleri Verdim. <a:psoprime_emoji:1084585759195021422>"
    );
